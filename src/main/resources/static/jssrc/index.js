	...
	callback: {
		...
		beforeClick: function(treeId, treeNode) {
			...
			if (treeNode.parent == 'true') {
				...
				//window.open(basePath+"/nodebook/view/"+treeNode.id);
				//demoIframe.attr("src", "/nodebook/view/"+treeNode.id);
	...
	$.ajax({
        ...
        success: function(data) {
           var t = $("#tree");
       	   ...
       	   //删除功能开关
		   t.setting.edit.showRemoveBtn = (data.otherData.allowdelete == 1);
        }
    });
}
...
$(document).ready(function() {
	...
	//设置高度
	$("#nodeTree").height((window.innerHeight * 0.80));
	$("#keepLink").height((window.innerHeight * 0.80));
	...
	$.ajax({
		...
		success: function(data) {
			...
			for(var i=0; i<len; i++){
				...
				$("#nodebooks").append('<div class="cell-s"><span class="numitem">'+(i +1)+' </span><a href="'+basePath+'/share/view/'+i_.id+'">'+i_.time +" "+ i_.nodeBookName+'</a></div>');
			}
...
function settings(){
	window.location.href= basePath + "/nodebook/mysetting";
